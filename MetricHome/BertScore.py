...
class EvaluateSummaries:
    ...
    def setup_directories(self):
        ...
        self.bert_score_dir = self.output_dir / "BertScore" / self.config['summaries_dir']
        self.bert_score_dir.mkdir(parents=True, exist_ok=True)
        ...
        for file_name in os.listdir(dir_path):
            ...
            if reference_path.exists() and summary_path.exists():
                try:
                    ...
                    logger.error(f"Failed to evaluate BertScore for {summary_path} against {reference_path}: {e}")

    def evaluate_and_save_results(self, summary, reference, dir_path, file_name):
        results = self.bert_score.evaluate_batch([summary], [reference])
        output_file = self.bert_score_dir / f"{dir_path.name}_{file_name}.json"
        with open(output_file, 'w', encoding='utf-8') as out_file:
            json.dump({ "bert_score": results }, out_file, ensure_ascii=False, indent=4)
