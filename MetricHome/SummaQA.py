...
class EvaluateSummaries:
    def __init__(self, config):
        ...
        self.summa_qa = SummaQAMetric()

    def setup_directories(self):
        self.data_dir = Path(self.config['data_dir'])
        self.references_dir = Path(self.config['data_dir'])
        self.output_dir = Path(self.config['output_dir'])
        self.summa_qa_dir = self.output_dir / "SummaQA" / self.config['summaries_dir']
        self.summa_qa_dir.mkdir(parents=True, exist_ok=True)
        ...
        for file_name in os.listdir(dir_path):
            ...
            if reference_path.exists() and summary_path.exists():
                try:
                    ...
                    logger.error(f"Failed to evaluate SummaQA for {summary_path} against {reference_path}: {e}")

    def evaluate_and_save_results(self, summary, reference, dir_path, file_name):
        results = self.summa_qa.evaluate_batch([summary], [reference])
        output_file = self.summa_qa_dir / f"{dir_path.name}_{file_name}.json"
        with open(output_file, 'w', encoding='utf-8') as out_file:
            json.dump({ "summa_qa": results }, out_file, ensure_ascii=False, indent=4)
