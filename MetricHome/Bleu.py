...
class EvaluateSummaries:
    ...
    def setup_directories(self):
        ...
        self.bleu_dir = self.output_dir / "Bleu" / self.config['summaries_dir']
        self.bleu_dir.mkdir(parents=True, exist_ok=True)
        ...
        for file_name in os.listdir(dir_path):
            ...
            if reference_path.exists() and summary_path.exists():
                try:
                    ...
                    logger.error(f"Failed to evaluate Bleu for {summary_path} against {reference_path}: {e}")

    def evaluate_and_save_results(self, summary, reference, dir_path, file_name):
        results = self.bleu.evaluate_batch([summary], [reference])
        output_file = self.bleu_dir / f"{dir_path.name}_{file_name}.json"
        with open(output_file, 'w', encoding='utf-8') as out_file:
            json.dump({ "bleu": results }, out_file, ensure_ascii=False, indent=4)
