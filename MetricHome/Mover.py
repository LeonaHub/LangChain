...
class EvaluateSummaries:
    def __init__(self, config):
        ...
        self.mover_score = MoverScoreMetric()

    def setup_directories(self):
        self.data_dir = Path(self.config['data_dir'])
        self.references_dir = Path(self.config['references_dir'])
        self.output_dir = Path(self.config['output_dir'])
        self.mover_score_dir = self.output_dir / "Mover" / self.config['summaries_dir']
        self.mover_score_dir.mkdir(parents=True, exist_ok=True)
        ...
        for file_name in os.listdir(dir_path):
            ...
            if reference_path.exists() and summary_path.exists():
                try:
                    ...
                    logger.error(f"Failed to evaluate MoverScore for {summary_path} against {reference_path}: {e}")

    def evaluate_and_save_results(self, summary, reference, dir_path, file_name):
        results = self.mover_score.evaluate_batch([summary], [reference])
        output_file = self.mover_score_dir / f"{dir_path.name}_{file_name}.json"
        with open(output_file, 'w', encoding='utf-8') as out_file:
            json.dump({ "mover_score": results }, out_file, ensure_ascii=False, indent=4)
